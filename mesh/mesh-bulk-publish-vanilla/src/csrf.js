/**
 * csrf.js — Minimal custom-header CSRF guard for BFF routes.
 *
 * WHY THIS EXISTS
 * ---------------
 * Our session cookie is `SameSite=None; Partitioned` because we are embedded
 * as a cross-site iframe (see `cookieUtils.js`). That means the browser no
 ...
 */
export function requireCsrfHeader(req, res) {
