...
export const transformAkeneoProduct = (akeneoProduct: AkeneoProduct, locale?: string | null, baseUrl?: string, thumbnailImageAttribute?: string): Product => {
  ...
  console.log({ akeneoProduct });
  const flattenedProduct = flattenProductValues(akeneoProduct);
  console.log({ flattenedProduct });
  // Debug: Log available attributes to help understand the data structure
  // console.log('Available product attributes:', Object.keys(akeneoProduct.values || {}));
  ...
  //   availableAttributes: Object.keys(akeneoProduct.values || {})
  // });
  ...
  const getMainImageUrl = (): string | undefined => {
    ...
    if (thumbnailImageAttribute) {
      ...
      if (flattenedImageValue && typeof flattenedImageValue === 'string') {
        console.log(`Found image in flattened attribute "${thumbnailImageAttribute}":`, flattenedImageValue);
      ...
      if (imageValue) {
        console.log(`Found image data in configured thumbnail attribute "${thumbnailImageAttribute}":`, imageValue);
        const imageUrl = getImageUrl(imageValue, baseUrl);
        if (imageUrl) {
          console.log(`Successfully constructed image URL from configured attribute: ${imageUrl}`);
          return imageUrl;
        }
      }
    }
    ...
    console.log('Potential image attributes found:', potentialImageAttributes);
    ...
    for (const attr of uniqueImageAttributes) {
      ...
      if (flattenedImageValue && typeof flattenedImageValue === 'string') {
        console.log(`Found image in flattened attribute "${attr}":`, flattenedImageValue);
        return flattenedImageValue;
      }
      ...
      if (imageValue) {
        console.log(`Found image data in attribute "${attr}":`, imageValue);
        const imageUrl = getImageUrl(imageValue, baseUrl);
        if (imageUrl) {
          console.log(`Successfully constructed image URL: ${imageUrl}`);
          return imageUrl;
        }
      }
    }
